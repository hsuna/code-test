...
const Api = ((document) => {
	...
	let util = {
		ajax(entity) {
			return new Promise((resolve, reject) => {
				...
				url = /(GET|DELETE)/i.test(entity.methods)?url+'?'+queryString:url;
		...
		jsonp(entity) {
			return new Promise((resolve, reject) => {
				...
				let url = entity.xhr.url
				let queryString = entity.xhr.queryString
				log.call(entity.debug, url + queryString)
	...
	let mixins = (namespace, mixinsConfig) => {
		...
		log.call(apiSet[namespace].debug, { ...apiSet[namespace],
			...mixinsConfig
		})
	}
	// 定义一个请求接口
	...
	let require = async(namespace = '', data = {}, config = {}) => {
		...
		entity.input = data
		log.call(entity.debug, '------------' + namespace + '------------')
		log.call(entity.debug, entity.input)
		log.call(entity.debug, '----------------------------------------------------')
